...
class GeometryMetrics:
    ...
    def aabb_iou(filtered_cloud: o3d.geometry.PointCloud,
        ...
        Measures macro-scale geometry preservation.

        Args:
            filtered_cloud: Filtered point cloud
            ground_truth: Ground truth reference cloud
    ...
    def voxel_iou(filtered_cloud: o3d.geometry.PointCloud,
        ...
        Args:
            filtered_cloud: Filtered point cloud
            ground_truth: Ground truth reference cloud
            voxel_size: Voxel grid resolution (meters)
    ...
    def chamfer_distance(filtered_cloud: o3d.geometry.PointCloud,
        ...
        Average of one-way distances: sum(nearest-neighbor distances) / total_points.
        Captures both directional errors (filtered→GT and GT→filtered).
        ...
        Returns:
            (chamfer_distance_meters, chamfer_distance_cm) tuple
        """
        try:
            ...
            # Build KD-tree for ground truth (scipy avoids Open3D KDTreeFlann segfault on macOS ARM)
            from scipy.spatial import KDTree
            gt_tree = KDTree(gt_pts)
            filtered_tree = KDTree(filtered_pts)

            # Find nearest GT point for each filtered point
            fwd_dists, _ = gt_tree.query(filtered_pts, k=1)
            chamfer_forward = fwd_dists.sum()

            # Find nearest filtered point for each GT point
            bwd_dists, _ = filtered_tree.query(gt_pts, k=1)
            chamfer_backward = bwd_dists.sum()

            # Mean Chamfer: average of all one-way distances
...
class StabilityMetrics:
    ...
    def centroid_displacement(filtered_cloud: o3d.geometry.PointCloud,
        ...
        Lower values indicate better centroid stability.
    ...
    def coordinate_distribution(filtered_cloud: o3d.geometry.PointCloud,
        ...
        Measures shape consistency (std dev of x, y, z after centering).
...
class ComprehensiveEvaluation:
    ...
    def evaluate(filtered_cloud: o3d.geometry.PointCloud,
        ...
        Args:
            filtered_cloud: Filtered point cloud
            ground_truth: Ground truth reference
            filter_name: Name of filter (for reporting)
            voxel_size: Voxel resolution for voxel IoU
            original_input_points: Original unfiltered input count (for accurate retention %)
        ...
        try:
            ...
            input_count = original_input_points if original_input_points is not None else len(ground_truth.points)
            # Compile results
            evaluation = {
                'filter': filter_name,
