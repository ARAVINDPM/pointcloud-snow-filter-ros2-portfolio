...
class RobustBenchmark:
    ...
    def run(self, func: Callable, *args, **kwargs) -> Tuple[float, Dict]:
        ...
        Returns:
            (median_time_seconds, stats_dict) tuple

        Raises:
            RuntimeError: If function fails during execution
        """
        logger.info(f"Benchmarking {func.__name__} ({self.repeats} repeats + {self.warmup} warmup)")
...
class FilterBenchmark:
    ...
    def benchmark_filter(self,
        ...
        Args:
            ...
            filter_func: Filter function(pcd, **kwargs) -> filtered_pcd
            filter_name: Name for reporting
    ...
    def benchmark_scaling(self,
        ...
        Args:
            base_pcd: Base point cloud
            filter_func: Filter function
            filter_name: Filter name
            scale_factors: Subsampling factors (1.0 = full size)
            repeats: Timing repeats per size

        Returns:
            Scaling analysis dictionary
        """
        ...
        for scale in sorted(scale_factors):
            # Subsample
            n_sample = int(len(base_points) * scale)
            if n_sample < 100:
            ...
            try:
                ...
                stats['microseconds_per_point'] = (median_time / len(sampled_pcd.points)) * 1e6
