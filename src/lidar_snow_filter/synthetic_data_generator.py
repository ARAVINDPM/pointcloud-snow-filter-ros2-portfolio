...
class SyntheticMannequinGenerator:
    ...
    def __init__(self, sensor: str = "livox", seed: int = 42):
        """
        Initialize generator with sensor parameters.

        Args:
            sensor: "livox" (0.1° H, 0.5° V) or "sick" (0.13° H, 0.13° V)
            seed: Random seed for reproducibility
        """
        self.sensor = sensor
        self.seed = seed
        # Use instance-local RNG instead of global np.random.seed()
        # Prevents state pollution between generators
        self.rng = np.random.default_rng(seed)

        # Sensor parameters (angular resolution in degrees)
        if sensor == "livox":
            self.h_res = 0.1  # Horizontal resolution
            self.v_res = 0.5  # Vertical resolution
            self.h_fov = 150  # Horizontal field of view
            self.v_fov = 25   # Vertical field of view
        elif sensor == "sick":
            self.h_res = 0.13
            self.v_res = 0.13
            self.h_fov = 270
    ...
    def generate_mannequin_points(self) -> np.ndarray:
        """
        Generate synthetic mannequin point cloud.

        Returns:
            Nx3 array of point coordinates (x, y, z) in cm
        """
        # Mannequin dimensions (cm)
        HEAD_RADIUS = 11
        TORSO_HEIGHT = 60
        TORSO_WIDTH = 22
        TORSO_DEPTH = 15
        ...
        z = 120 + HEAD_RADIUS * np.cos(v)  # Head at 120cm height
        points.append(np.column_stack([x, y, z]))

        # 2. Torso (elongated ellipsoid)
        n_torso = 5000
        u = self.rng.uniform(0, 2 * np.pi, n_torso)
        v = self.rng.uniform(-1, 1, n_torso)  # -1 to 1 for ellipsoid height
        x = TORSO_WIDTH * np.cos(u)
        y = TORSO_DEPTH * np.sin(u)
        z = 45 + (TORSO_HEIGHT / 2) * v  # Centered at 45cm
        points.append(np.column_stack([x, y, z]))
        ...
        z = 80 + ARM_RADIUS * np.sin(theta)  # Arm at 80cm height
        points.append(np.column_stack([x, y, z]))
        ...
        # Add slight jitter to simulate real sensor noise (±0.5 cm)
    ...
    def apply_sensor_quantization(self, points: np.ndarray) -> np.ndarray:
        """
        Quantize points to sensor resolution.
        ...
        theta = np.arctan2(points[:, 1], points[:, 0])  # azimuth
        phi = np.arctan2(points[:, 2], np.sqrt(points[:, 0]**2 + points[:, 1]**2))  # elevation
...
class SnowContaminationSimulator:
    ...
    def contaminate(
        ...
        **LIMITATION:** This model injects uniform random points, NOT a physical snow
        distribution. Real snow exhibits non-uniform clustering, occlusion patterns,
        and height-dependent density. Use this for algorithmic validation only.

        For realistic snow validation, use a separately authorized real sensor
        dataset. The public export only includes synthetic generation.

        Args:
            pcd: Clean point cloud
            snow_density: Fraction of outliers to add (0.0-1.0)
            outlier_radius: Radius of contamination region around object (cm)

        Returns:
            Contaminated point cloud (object + uniform random outliers)
        """
        points = np.asarray(pcd.points)
        n_points = len(points)
        n_outliers = int(n_points * snow_density / (1 - snow_density))

        # Generate outliers in uniform cube around object center
        outliers = self.rng.uniform(-outlier_radius, outlier_radius, (n_outliers, 3))
        # Bias outliers toward the object
        center = np.mean(points, axis=0)
        outliers = outliers + center

        # Combine and create new point cloud
...
def main():
    ...
    parser.add_argument("--num_scans", type=int, default=5, help="Number of scans to generate")
    ...
    parser.add_argument("--contaminate", action="store_true", help="Also generate contaminated scans")
    ...
    # Generate scans (each instance manages its own RNG state)
    ...
    for i in range(args.num_scans):
        ...
        # Generate contaminated scan if requested
        if args.contaminate:
            ...
            logger.info(f"  Saved contaminated: {contam_path} ({len(contaminated_pcd.points)} points)")
